...
public class ApiUserService implements UserDetailsService {
    ...
    public ApiUser save(ApiUserPostRequestBody apiUserPostRequestBody) {
        ...
        ApiUser apiUser = apiUserRepository.save(
                ApiUser.builder()
                        ...
                        .build());
        emailConfirmationService.generateToken(apiUser, 1440);
        return apiUser;
    }
    ...
    public ApiUserDTO replace(ApiUserPutRequestBody apiUserPutRequestBody) {
        ...
        return new ApiUserDTO(apiUserLoaded.getId(),
                apiUserLoaded.getUsername(),
                apiUserLoaded.getEmail(),
                apiUserLoaded.getUrlPhoto(),
                apiUserLoaded.isAdmin(),
                apiUserLoaded.isEnabled());
    }
    ...
    public void enable(String token) {
        ApiUser apiUser = emailConfirmationService.findByToken(token).getApiUser();
        apiUserRepository.save(
                ApiUser.builder()
                        ...
                        .urlPhoto(apiUser.getUrlPhoto())
                        .admin(apiUser.isAdmin())
                        .enabled(true)
                        .build()
        );
        kafkaService.sendUserRequestBodyToKafka(new UserStructure(apiUser.getUsername(), apiUser.getEmail()));
    }
